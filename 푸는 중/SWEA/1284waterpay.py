...
for i in range(N) :
    money_zip = []
    a_money = data_zip[i][0] * data_zip[i][4]
    money_zip.append(a_money)
    b_money = 0
    if data_zip[i][2] < data_zip[i][4] :
        b_money = b_money + data_zip[i][1] + data_zip[i][3] * (data_zip[i][4] - data_zip[i][2])
    else :
        b_money = b_money + data_zip[i][1]
    money_zip.append(b_money)
    # print(money_zip)
    print(f'#{i+1} {min(money_zip)}')
